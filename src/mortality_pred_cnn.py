...
class PatientDataset(Dataset):
    def __init__(self, data: list):
        ...
        for sid in self.data['sample_id'].unique():
            mortality = self.data.loc[self.data['sample_id']==sid, 'mortality'].iloc[-1]
            self.labels.append(mortality)
...
class LigthningModuleBase(LightningModule):
    ...
    def _step(self, batch, batch_idx, split: str):
        ...
        for metric in self.metrics:
            ...
            #         y.int().cpu().detach().numpy(),
            #         pred.cpu().detach().numpy())
            #     auprc = auc(recall, precision)
            #     outputs[f'{split}_' + metric] = auprc
            # else:
    ...
    def _dataloader(self, split: str):
        if split == 'train':
            return DataLoader(self.dataset(self.data[split]),
                              sampler=ImbalancedDatasetSampler(self.dataset(self.data[split])),
...
def train(args: argparse.Namespace):
    ...
    model_name = \
        ...
        f"{config['dropout_dense']}dropout"
...
def eval_cnn_mortality(model_folder, data_test, pred_task='los'):
    ...
    if args['datatype'] == 'raw':
        test_dataset = PatientDatasetRaw(data_test_)
    elif args['datatype'] == 'encoded':
        test_dataset = PatientDatasetEncoded(data_test_)
    else:
        raise ValueError("Unknown data type")
